...
float NeuralNetwork::backward_input_time(int i){
    ...
    for (auto output_id = outputs_id.begin(); output_id != outputs_id.end(); output_id ++){
        ...
        if (output_core_loc.chip_id != rcv_core_loc.chip_id) { 
            ...
            if (!is_connected) find_subconnect_points(output_layer.cores, rcv_layer.cores, connect_points);    
            if (connect_points.size() < 4){
                connect_points.push_back(connect_points[0]);
                connect_points.push_back(connect_points[1]);
            }
...
float NeuralNetwork::backward_weight_sync_time(int in_idx, int out_idx){

    PhysicalLayer input_layer = layers[in_idx];

    CoreLoc input_upleft = input_layer.cores.coreloc;
    CoreLoc input_upright = {input_upleft.chip_id, input_upleft.row_id, input_upleft.col_id + input_layer.cores.cols};
    CoreLoc input_downleft = {input_upleft.chip_id, input_upleft.row_id + input_layer.cores.rows, input_upleft.col_id};
    CoreLoc input_downright = {input_upleft.chip_id, input_upleft.row_id + input_layer.cores.rows, input_upleft.col_id + input_layer.cores.cols};
    vector<CoreLoc> input_vertices;
    input_vertices.push_back(input_upleft);
    input_vertices.push_back(input_upright);
    input_vertices.push_back(input_downleft);
    input_vertices.push_back(input_downright);

    int input_max_dist = 0;
    for (int k = 0; k < 4; k++){
        int dist = core_dist(device.net_port_loc, input_vertices[in_idx]);
        if (dist > input_max_dist){
            input_max_dist = dist;
        }
    }

    int input_shard_size = input_layer.is_trans ? ceil(input_layer.weight_size_row/input_layer.cores.cols) * ceil(input_layer.weight_size_col/input_layer.cores.rows): ceil(input_layer.weight_size_row/input_layer.cores.rows) * ceil(input_layer.weight_size_col/input_layer.cores.cols); 

    PhysicalLayer output_layer = layers[out_idx];

    CoreLoc output_upleft = output_layer.cores.coreloc;
    CoreLoc output_upright = {output_upleft.chip_id, output_upleft.row_id, output_upleft.col_id + output_layer.cores.cols};
    CoreLoc output_downleft = {output_upleft.chip_id, output_upleft.row_id + output_layer.cores.rows, output_upleft.col_id};
    CoreLoc output_downright = {output_upleft.chip_id, output_upleft.row_id + output_layer.cores.rows, output_upleft.col_id + output_layer.cores.cols};
    vector<CoreLoc> output_vertices;
    output_vertices.push_back(output_upleft);
    output_vertices.push_back(output_upright);
    output_vertices.push_back(output_downleft);
    output_vertices.push_back(output_downright);

    int output_max_dist = 0;
    for (int k = 0; k < 4; k++){
        int dist = core_dist(device.net_port_loc, output_vertices[out_idx]);
        if (dist > output_max_dist){
            output_max_dist = dist;
        }
    }

    int output_shard_size = output_layer.is_trans ? ceil(output_layer.weight_size_row/output_layer.cores.cols) * ceil(output_layer.weight_size_col/output_layer.cores.rows): ceil(output_layer.weight_size_row/output_layer.cores.rows) * ceil(output_layer.weight_size_col/output_layer.cores.cols); 
